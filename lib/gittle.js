...
Gittle.prototype.commit = function(name, email, msg, files) {
    ...
    var setIdentity = function() {
        return this.identity(newIdentity);
    };

    var identity = this.identity;

    var commit = _.partial(Gittle.super_.prototype.commit, msg);

    return this._stage_files(files).then(Q())
    .then([
        // Get current identity and set it aside
        identity,
        setAuthor,
