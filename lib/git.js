...
module.exports = Git = function(git_dir, dot_git) {
    ...
    execGit = function(useTerminal, command, options, args, creds) {
        var bash, d, err, stdout, termArgs;
        ...
        if (useTerminal) {
            ...
            pterm.on('data', function(data) {
                ...
                if (prompt.indexOf('username') > -1) {
                    ...
                    err = errors.error(
                        (prompt.indexOf("authentication") > -1) ? errors.codes.AUTH : errors.codes.UNKNOWN,
            ...
            var pterm = exec(bash, {
                ...
                if (err) return d.reject(errors.error(error.codes.UNKNOWN, err));
                d.resolve({
                    'stdout': stdout,
                    'stderr': stderr,
                    'bash': bash
